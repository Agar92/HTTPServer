...
HTTPGetRequest::HTTPGetRequest(boost::asio::io_context& io_context,
                               std::string host, std::string clipURL,
                               HTTPRequestDataReceived receivedCB,
                               HTTPRequestComplete     completeCB)
    : m_io_service(io_context), m_host(host), m_relativeURL(clipURL),
      m_socket(m_io_service), m_resolver(m_io_service),
...
// URI should be in format such as "/index.html"
...
void HTTPGetRequest::ReadData()
{
  boost::asio::async_read(
      ...
      [this](boost::system::error_code ec, std::size_t bytes_transferred)
      {
        ...
        std::cout << "HTTPGetRequest::ReadData(...) #" << THIS_FUNCTION_CALLS_COUNTER
                  << " Read " << bytes_transferred << " Bytes"
                  << std::endl;
        size_t size = m_response.size();
        /*
        std::string s( (std::istreambuf_iterator<char>(&m_response)),
        std::istreambuf_iterator<char>() ); size_t index=s.find("\r\n\r\n");
